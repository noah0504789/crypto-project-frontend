...
- 전송: STOMP over native WebSocket, 엔드포인트 `GATEWAY_URL/ws-native`. `createStompClient()`가 HTTP(S) Gateway URL을 WS(S)로 변환하고 **핸드셰이크 URL 쿼리(`?access_token=`)로 토큰을 실어** `Client`를 만든다(자동 재연결 `reconnectDelay: 5000`, 재연결마다 최신 토큰).
